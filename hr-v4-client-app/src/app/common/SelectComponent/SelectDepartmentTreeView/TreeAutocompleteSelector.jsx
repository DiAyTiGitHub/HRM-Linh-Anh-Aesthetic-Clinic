import { Checkbox, Chip, IconButton, InputAdornment, Paper, TextField, makeStyles } from "@material-ui/core";
...
const StyledTreeItem = withStyles((theme) => ({
    ...
    label: {
        padding: "4px 0",
        // Thêm style cho label khi được chọn
        // "&.Mui-selected": {
...
const useStyles = makeStyles((theme) => ({
    ...
    inputContainer: {
        ...
        "& .MuiOutlinedInput-root": {
            ...
            // overflow: "auto", // Thêm thanh cuộn khi vượt quá
        },
    ...
    treeViewContainer: {
        ...
        zIndex: 1000, // Đảm bảo hiển thị trên các phần tử khác
...
const TreeAutocompleteSelector = (props) => {
    return (
...
function MyTreeAutocomplete({
    ...
    // Effect to sync with formik field value
    // Then in useEffect:
    useEffect(() => {
        // Đồng bộ selectedItems với giá trị từ Formik khi form được reset
        if (!isEqual(field.value, selectedItems)) {
            setSelectedItems(field.value || (multiple ? [] : null));
        }
    ...
    const handleNodeSelect = (event, nodeId) => {
        ...
        if (iconContainer || collapseIcon || expandIcon || endIcon || readOnly) {
            return;
        }
        ...
        return nodes.map((node) => {
            ...
            return (
                <StyledTreeItem
                    ...
                    label={
                        <div style={{ display: "flex", alignItems: "center" }}>
                            <Checkbox
                                ...
                                color='primary'
                                onClick={(e) => handleNodeSelect(e, node[idField])} // Ngăn sự kiện lan ra TreeItem
                            />
                            <span
                    ...
                    disabled={getOptionDisabled ? getOptionDisabled(node) : false}>
    ...
    const getDisplayValue = () => {
        ...
        if (multiple) {
            return selectedItems.length > 0 ? `${selectedItems.length} item(s) selected` : "";
        }
        return selectedItems[displayName] || "";
    };
...
export default React.memo(TreeAutocompleteSelector);
