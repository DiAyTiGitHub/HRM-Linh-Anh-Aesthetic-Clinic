...
function PreviewConfigBoard(props) {
    ...
    }, [data?.resultItemGroups, data?.resultItems]);
    ...
    return (
        <GlobitsPopupV2
            ...
            open={isOpenPreview}
            noDialogContent
            title={"Xem bảng lương đang cấu hình"}
                        ...
                        <Grid item xs={12}>
                            <section className={classes.tableContainer}>
                                <table className={`${classes.table} w-100`} style={{ tableLayout: "auto" }}>
                                    ...
                                    <tbody>
                                        {
                                            data?.staffs?.map(function (staff, index) {
                                                return (
                                                    <tr key={index} className='row-table-body row-table-no_data'>
                                                        {
                                                            data?.resultItems?.map(function (item, jndex) {
                                                                let previewValue = null;
                                                                if (item?.code == LocalConstants.SalaryItemCodeSystemDefault.HO_VA_TEN_NV_SYSTEM.value) {
                                                                    previewValue = staff?.displayName;
                                                                }
                                                                else if (item?.code == LocalConstants.SalaryItemCodeSystemDefault.MA_NV_SYSTEM.value) {
                                                                    previewValue = staff?.staffCode;
                                                                }
                                                                else if (item?.code == LocalConstants.SalaryItemCodeSystemDefault.STT_SYSTEM.value) {
                                                                    previewValue = index + 1;
                                                                }

                                                                // console.log("preview cell item: ", item);

                                                                return (
                                                                    <td key={index + "_" + jndex} className={`px-6 no-wrap-text ${(item?.code == LocalConstants.SalaryItemCodeSystemDefault.STT_SYSTEM.value) ? "text-center" : ""}`}>
                                                                        {
...
export default memo(observer(PreviewConfigBoard));