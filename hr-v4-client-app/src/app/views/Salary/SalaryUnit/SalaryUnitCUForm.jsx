...
function SalaryUnitCUForm({readOnly}) {
    ...
    async function handleSaveForm(values) {
        await saveSalaryUnit(values);
        await pagingSalaryUnit();
    }

    const [initialValues , setInitialValues] = useState(selectedSalaryUnit);

    useEffect(function () {
        setInitialValues(selectedSalaryUnit);
    } , [selectedSalaryUnit , selectedSalaryUnit?.id])
    ...
    return (
        <GlobitsPopupV2
            ...
            noDialogContent
            title={openViewPopup ? (t("Xem chi tiết") + ' ' + "đơn vị thời gian tính lương") : ((selectedSalaryUnit?.id ? t("general.button.edit") : t("general.button.add")) + ' ' + "đơn vị thời gian tính lương")}
...
export default memo(observer(SalaryUnitCUForm));