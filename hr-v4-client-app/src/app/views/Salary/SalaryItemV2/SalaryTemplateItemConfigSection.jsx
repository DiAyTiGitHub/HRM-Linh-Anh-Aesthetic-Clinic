...
function SalaryTemplateItemConfigSection(props) {
    ...
    return (
        <Grid container spacing={2}>
            <Grid item xs={12}>
                <FieldArray name="salaryTemplateItemConfigs">
                    {({insert , remove , push}) => (
                        <>
                            <Grid item xs={12}>
                            ...
                            <Grid item xs={12}>
                                <section className={classes.tableContainer}>
                                    <table className={`w-100 ${classes.table}`}>
                                        ...
                                        <tbody>
                                        {values?.salaryTemplateItemConfigs?.length > 0 ? (
                                            values?.salaryTemplateItemConfigs?.map((order , index) => (
                                                <SalaryTemplateItemConfigSectionItem
                                                    key={index}
                                                    index={index}
                                                    order={order}
                                                    salaryTemplateItemConfigs={values?.salaryTemplateItemConfigs}
                                                    nameSpace={`salaryTemplateItemConfigs[${index}]`}
                                                    remove={() => remove(index)}
                                                    push={() => push(index)}
                                                    //  disabled={!hasEditPermission}
                                                    readOnly={readOnly}
...
const SalaryTemplateItemConfigSectionItem = memo(({
                                                      index ,
                                                      salaryTemplateItemConfigs ,
                                                      order ,
                                                      remove ,
                                                      push ,
                                                      nameSpace ,
    ...
    const handleTabKeyPress = (event) => {
        if (event.key === 'Tab') {
            if ((Number(index) === Number(salaryTemplateItemConfigs?.length - 1))) {
                push();
            }
        }
    };
    ...
    return (
        <tr className='row-table-body' key={index}>
            <td>
                <GlobitsTextField
                    name={withNameSpace("thresholdValue")}
                    readOnly={readOnly}
                />
            </td>
            <td>
