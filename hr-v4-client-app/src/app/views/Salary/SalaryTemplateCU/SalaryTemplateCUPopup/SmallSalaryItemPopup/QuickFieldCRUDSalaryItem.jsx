...
function QuickFieldCRUDSalaryItem(props) {
    const {
    ...
    function handleChooseItem(item) {
        if (item?.calculationType) item.calculationType = Number(item?.calculationType);

        setFieldValue("salaryItem", item);
        setFieldValue("displayName", item?.name);
        setFieldValue("description", item?.description);

        if (item?.calculationType == LocalConstants.SalaryItemCalculationType.USING_FORMULA.value) {
            setFieldValue("usingFormula", item?.formula);
        }
        ...
        <>
            <div className="p-12">
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                        <SalaryItemV2List
                            selectedItem={values?.salaryItem}
                            isOnChoosingMode
...
export default memo(observer(QuickFieldCRUDSalaryItem));