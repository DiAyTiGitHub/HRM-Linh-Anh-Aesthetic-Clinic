...
function RecruitmentPlanItemSection() {
    ...
    return (<Grid container spacing={2}>
        <Grid item xs={12}>
            <FieldArray name="recruitmentPlanItems">
                {({insert, remove, push}) => (<>
                    <Grid item xs={12}>
                            ...
                            <Grid item xs={12} sm={4}>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        Thêm mới chức danh cần tuyển dụng
                                    </Button>
                                </ButtonGroup>
                            </Grid>
                        </Grid>
                        ...
                        <section className={classes.tableContainer}>
                            <table className={`w-100 ${classes.table}`}>
                                ...
                                <tr className={classes.tableHeader}>
                                    <th align="center" style={{width: "33%"}}>Chức danh cần tuyển</th>
                                ...
                                <tbody>
                                {values?.recruitmentPlanItems?.length > 0 ? (values?.recruitmentPlanItems?.map((order, index) => (
                                    <RecruitmentPlanItem
                                        key={index}
                                        index={index}
                                        order={order}
                                        KPIItems={values?.recruitmentPlanItems}
                                        nameSpace={`recruitmentPlanItems[${index}]`}
                                        remove={() => remove(index)}
                                        push={() => push(index)}
...
const RecruitmentPlanItem = memo((props) => {
    const {
    ...
    return (<tr className='row-table-body' key={index}>
        ...
        />
        <td>
            <GlobitsNumberInput name={withNameSpace("quantity")}/>
        </td>
...
export default memo(observer(RecruitmentPlanItemSection));
