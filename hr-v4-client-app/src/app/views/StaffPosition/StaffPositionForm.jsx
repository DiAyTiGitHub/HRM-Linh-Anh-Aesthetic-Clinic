...
export default function StaffPositionForm() {
    ...
    const [formValues, setFormValues] = useState(initialStaffPosition);

    useEffect(() => {
        if (selectedStaffPosition) {
            setFormValues(selectedStaffPosition);
        } else {
            setFormValues(initialStaffPosition);
        }
    }, [selectedStaffPosition]);

    const handleSubmit = async (values) => {
        await saveStaffPosition(values);
    }
    ...
    return (
        <GlobitsPopupV2
            ...
            title={
                <span>
                    {selectedStaffPosition?.id ? t("general.button.add") : t("general.button.edit")}{" "}
