...
function StaffWorkSchedulePopup({open , handleClose}) {
    ...
    useEffect(() => {
        if (open && values?.requestStaff && values?.requestStaff?.id) {
            const newSearchObject = {
                ... searchObject ,
                staffId:values?.requestStaff.id ,
                staff:values?.requestStaff ,
                pageIndex:0 ,
    ...
    } , [open , values?.requestStaff , values?.fromDate , values?.toDate]);
    ...
    return (
        <GlobitsPopupV2
            ...
            <Grid container className='p-12'>
                <Grid item xs={12}>
                    {console.log("searchObject.pageIndex" , searchObject.pageIndex)}
                    <GlobitsTable
                        selection={false}
