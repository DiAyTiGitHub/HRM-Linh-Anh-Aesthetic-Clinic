...
import { DnDProvider } from "../OrganizationDiagram/components/Context/DnDContext";
import {
    convertEdges,
    convertNodes,
    edgeTypes,
    getLayoutedElements,
    nodeTypes,
} from "../OrganizationDiagram/components/utils";
import "./styles.css";
import DepartmentSidebar from "../OrganizationDiagram/components/Toolbox/DepartmentSidebar";
import { getDepartment } from "../Department/DepartmentService";
import { useParams } from "react-router-dom/cjs/react-router-dom.min";
import { toast } from "react-toastify";
import { getChartByDepartmentId } from "../OrganizationalChartData/OrganizationalChartDataService";
...
const DnDFlow = observer(({ settings, orgChartData, setOrgChartData, showAll }) => {
    // const { id } = useParams();
    ...
    const findNode = () => {
        ...
        const currentPositionId = values?.objectId;
    ...
    useEffect(() => {
        if (departmentId) {
            getDepartment(departmentId)
                .then((result) => {
                    console.log(result);
                    setFieldValue("department", result.data);
                    setFieldValue("departmentId", result.data.id);
                    setFieldValue("objectId", result.data.id);
                    return result.data;
                })
                .then((result) => {
                    let dto = { ...values };
                    ...
                    if (result?.id) {
                        dto = { ...dto, departmentId: result?.id };
                    }
    ...
    return (
        <div className='dndflow' style={{ height: "100%" }}>
            ...
            {showAll && <DepartmentSidebar showSidebar={showSidebar} />}
        </div>
    );
});

const ProfileDiagramIndex = ({ settings, orgChartData, setOrgChartData, showAll }) => {
    return (
        <ReactFlowProvider>
            <DnDProvider>
                <DnDFlow
