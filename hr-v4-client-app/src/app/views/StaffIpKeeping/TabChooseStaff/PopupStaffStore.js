...
export default class PopupStaffStore {
    ...
    addSelectedStaff = (staff) => {
        this.selectedStaffs.push(staff);
    }

    removeSelectedStaff = (staffId) => {
        const newList = this.selectedStaffs.filter(item => item.id !== staffId);
        this.selectedStaffs = newList;
    }

    existsSelectedStaff = (staffId) => {
        return this.selectedStaffs.some(item => item.id === staffId);
    }

    selectAllStaffs = (staffList) => {
        this.selectedStaffs = staffList;
    }
