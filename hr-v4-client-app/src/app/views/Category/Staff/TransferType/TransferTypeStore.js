...
import { createDiscipline, deleteDiscipline, deleteMultipleDiscipline, getByIdDiscipline, pagingDiscipline, updateDiscipline } from "./TransferTypeService";
toast.configure({
  autoClose: 2000,
  draggable: false,
  limit: 3,
...
export default class TransferTypeStore {
  examCategoryList = [];
  roles = [];
  ...
  pagingRecruitment = async () => {
    ...
    try {
      const data = await pagingDiscipline(searchObject);
  ...
  getExamCategory = async (id) => {
    if (id != null) {
      try {
        ...
        const data = await getByIdDiscipline(id);
  ...
  saveOrUpdate = async (examCategory) => {
    try {
      await createDiscipline(examCategory);
      if(examCategory?.id) {
  ...
  handleConfirmDeleteMultiple = () => {
    ...
    deleteMultipleDiscipline(listId)
      .then(({data}) => {
