...
import {
    createContractType,
    deleteContractType,
    deleteMultipleContractType,
    editContractType, getAllContractType,
    getContractType,
    pagingContractTypes
...
export default class ContractTypeStore {
    ...
    pagingRecruitment = async () => {
        ...
        try {
            const data = await pagingContractTypes(searchObject);
    ...
    saveOrUpdate = async (examCategory) => {
        try {
            if (examCategory?.id) {
                await editContractType(examCategory)
            } else {
                await createContractType(examCategory);
            }
            if (examCategory?.id) {
    ...
    handleConfirmDeleteMultiple = () => {
        ...
        deleteMultipleContractType(listId).then(({data}) => {
    ...
    getAllContractType = async () => {
        ...
        if (response.status === HttpStatus.OK){
            this.listContractType = response.data
        }
    }
}
