...
function ChooseSingleDepartmentList(props) {
    const {
        handleClose,
        name
    } = props;
    ...
    async function handleSelectItem(_, department) {
        if (values[name]?.id === department?.id) {
            setFieldValue([name], null);
        } else {
            setFieldValue([name], department);
            handleClose();
        }
    };
...
export default memo(observer(ChooseSingleDepartmentList));
