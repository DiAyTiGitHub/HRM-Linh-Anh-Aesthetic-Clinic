...
function OvertimeRequestList () {
  ...
  const columns = [
...

    {
      ...
      minWidth:"150px",
      render:row => {
        return <span>{row?.staffWorkSchedule?.staff?.displayName}</span>
      }
    },
    {
      title:"Đơn vị",
    ...
    },
    {
      ...
      align:"center",
      minWidth:"150px",
      render:row => <span className="px-2">{getDate (row?.staffWorkSchedule?.workingDate)}</span>
    },

    {
      ...
      render:(rowData) => {
        const {staffWorkSchedule} = rowData;
        const {shiftWork} = staffWorkSchedule;
...

    {
      ...
      minWidth:"150px",
      render:row => {
        if (row?.staffWorkSchedule?.firstCheckIn)
          return (
              <span className="px-2">{formatDate ('HH:mm', row?.staffWorkSchedule?.firstCheckIn)}</span>
          );
...

    {
      ...
      minWidth:"150px",
      render:row => {
        if (row?.staffWorkSchedule?.lastCheckout)
          return (
              <span className="px-2">{formatDate ('HH:mm', row?.staffWorkSchedule?.lastCheckout)}</span>
          );
    ...
    },
    {
      ...
      minWidth:"150px",
      render:row => {
        return <span>{(row?.staffWorkSchedule?.otEndorser?.displayName || "") + " - " + (row?.staffWorkSchedule?.otEndorser?.staffCode || "")}</span>
      }
    },
  ];
...
export default memo (observer (OvertimeRequestList));
