...
import ShiftRegistrationList from "./ShiftRegistrationList";
import ShiftRegistrationListUpdatePopup from "./ShiftRegistrationListUpdatePopup";
import ShiftRegistrationToolbar from "./ShiftRegistrationToolbar";
...
function ShiftRegistrationIndex() {
  ...
  return (
    <div className='content-index'>
      <div className='index-breadcrumb py-6'>
        <GlobitsBreadcrumb
          routeSegments={[{ name: t("navigation.timeKeeping.title") }, { name: t("navigation.shiftRegistration.title") }]} />
      </div>
      <Grid container spacing={2}>
        <Grid item xs={12} className='index-card'>
          <ShiftRegistrationToolbar />
        </Grid>

        <Grid item xs={12} className='index-card'>
          <TabsComponent value={searchObject?.status} handleChange={(_, tabIndex) => handleChangeTabIndex(tabIndex)}
            tabList={tabList} />
      ...
      {openCreateEditPopup && <ShiftRegistrationForm />}
      ...
      {openConfirmDeletePopup && (
        <GlobitsConfirmationDialog open={openConfirmDeletePopup} onConfirmDialogClose={handleClose}
          ...
          text={t("confirm_dialog.delete.text")} agree={t("confirm_dialog.delete.agree")}
      ...
      {openConfirmUpdateStatusPopup && <ShiftRegistrationListUpdatePopup />}
    </div>
  );
}

export default memo(observer(ShiftRegistrationIndex));
