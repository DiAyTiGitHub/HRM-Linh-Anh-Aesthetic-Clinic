...
import ShiftChangeRequestFilter from "./ShiftChangeRequestFilter";
...
function ShiftChangeRequestIndexToolbar () {
  ...
  return (<Formik enableReinitialize={true} initialValues={searchObject} onSubmit={handleFilter}
                  validationSchema={validationSchema}>
        {({resetForm, values, setFieldValue, setValues}) => {
          const nonApprovedItems = listChosen?.filter ((item) => item.approvalStatus !== LocalConstants.ShiftChangeRequestApprovalStatus.APPROVED.value);
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12}>
                      <ButtonGroup color='container' aria-label='outlined primary button group'>
                        ...
                        {canApproveOrUnApprove && (<Button
                                disabled={!nonApprovedItems?.length}
                                startIcon={<DoneAll/>}
                                onClick={() => handleOpenConfirmUpdateStatusPopup (LocalConstants.ShiftChangeRequestApprovalStatus.APPROVED.value)}>
                              Duyệt
                            </Button>)}
                        {canApproveOrUnApprove && (<Button
                                disabled={!nonApprovedItems?.length}
                                startIcon={<ThumbDown/>}
                                onClick={() => handleOpenConfirmUpdateStatusPopup (LocalConstants.ShiftChangeRequestApprovalStatus.NOT_APPROVED.value)}>
                              Không duyệt
                            </Button>)}
                        {canApproveOrUnApprove && (<Button
...
export default memo (observer (ShiftChangeRequestIndexToolbar));
