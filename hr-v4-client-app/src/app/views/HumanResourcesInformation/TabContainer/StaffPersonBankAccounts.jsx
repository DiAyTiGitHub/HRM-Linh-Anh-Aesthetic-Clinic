...
import { pagingPersonBankAccount } from "app/views/PersonBankAccount/PersonBankAccountService";
...
function StaffPersonBankAccounts() {
  ...
  const [personBankAccounts, setPersonBankAccounts] = useState(null);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    fetchPersonBankAccounts();
  }, [id]);

  const fetchPersonBankAccounts = async () => {
    if (!id) return;
    setIsLoading(true);
    try {
      ...
      setPersonBankAccounts(data?.content);
    } catch (error) {
    ...
    <>
      ...
      <Grid container spacing={2} className={classes.root}>
        ...
        <Grid item xs={12}>
          {isLoading ? (
            <LoadingTabSkeleton />
          ) : personBankAccounts && personBankAccounts.length > 0 ? (
            <GlobitsTable
              data={personBankAccounts}
              columns={columns}
              maxWidth="100%"
              nonePagination
              selection={false}
      ...
      {openCreateEditPopup && (
        <PersonBankAccountCUForm
          person={values}
          handleAfterSubmit={fetchPersonBankAccounts}
        />
      )}
...
export default memo(observer(StaffPersonBankAccounts));
